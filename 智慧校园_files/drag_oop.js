...
function Drag(ele){//构造函数，ele表示被拖拽的元素
	...
	//on2(ele,"mousedown",this.down);//this.down,this.down();
	on2(ele,"mousedown",this.DOWN);//事件里的this的原则是：这个方法绑定给谁，当这个方法在执行的时候，this就是谁
...
Drag.prototype.down=function(e){
	...
	if(this.ele.setCapture){
		this.ele.setCapture();
		on2(this.ele,"mousemove",this.MOVE);
		on2(this.ele,"mouseup",this.UP);
	}else{
		on2(document,"mousemove",this.MOVE);
		on2(document,"mouseup",this.UP);
	}
	e.preventDefault();
