...
(function( $ ){
    ...
    var WorldMap = function(params) {
		...
		for(var key in mapData.pathes) {
			...
			path.setOpacity(this.color);//aaa
...
//				$(params.container).trigger('regionMouseOut.jvectormap', [code]);
	...
	WorldMap.prototype = {
		...
		setOpacity: function(key, color) {
				if (typeof key == 'string') {
				} else {
					var colors = key;
					for (var code in colors) {
						if (this.countries[code]) {
							this.countries[code].setOpacity(colors[code]);	
						}
					}
				}
        ...
        reset: function() {
			...
			for(var key in this.countries) {
                this.countries[key].setOpacity(WorldMap.defaultOpacity);
            }
